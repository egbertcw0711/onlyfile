...
if not restore:
    with train_graph.as_default():
        ...
        for j in range(batch_size):
            ...
            cos_dist = tf.where(tf.is_nan(cos_dist),tf.zeros_like(cos_dist)+1,cos_dist)
    ...
    for e in range(1,epochs+1):
        ...
        for batch_index in get_batches(train,batch_size):
            ...
            for i in batch_index:
                ...
                train_color[counter,:,:,:] = ((train_color[counter,:,:,:] / 255) - 0.5) * 2
                train_mask[counter,:,:,:] = ((train_mask[counter,:,:,:] / 255) - 0.5) * 2
                train_normal[counter,:,:,:] = ((train_normal[counter,:,:,:] / 255) - 0.5) * 2
            ...
            if num_batches % 300 == 0:
                ...
                for index in get_batches(test,batch_size):
                    ...
                    for k in index:
                        ...
                        validation_color[cnt,:,:,:] = ((validation_color[cnt,:,:,:] / 255) - 0.5) * 2
                        validation_mask[cnt,:,:,:] = ((validation_mask[cnt,:,:,:] / 255) - 0.5) * 2
                        validation_normal[cnt,:,:,:] = ((validation_normal[cnt,:,:,:] / 255) - 0.5) * 2
                    ...
                    tmp = 0
                    for k in index:
                        image=Image.fromarray((255.0*(results[tmp,:,:,:]+1)).astype(np.uint8))
